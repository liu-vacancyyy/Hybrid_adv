...
for i in range(1):
    ...
    ego_run_dir = "/home/a/demo/NeuralPlane_stable_V1/scripts/runs/2026-02-01_17-05-09_Control_rc_HYBRID_ppo_v1/episode_260/"
    ego_run_dir = "/home/a/demo/NeuralPlane_stable_V1/scripts/runs/2026-02-02_17-22-22_Control_rc_HYBRID_ppo_v1/episode_320/"
    ego_run_dir = "/home/a/demo/NeuralPlane_stable_V2/scripts/runs/2026-03-17_21-06-00_Control_rc_HYBRID_ppo_v1/episode_260/"

    device = "cuda:0"
    config = "rc"

    env = ControlEnv(num_envs=1, config=config, model='HYBRID', random_seed=5, device=device)
    ...
    vx_buf = np.mean(_t2n(vx))
    vy_buf = np.mean(_t2n(vy))
    vz_buf = np.mean(_t2n(vz))
    ...
    while True:
        ...
        vx, vy = env.model.get_ground_speed()
        vz = env.model.get_climb_rate()
        vx_buf = np.hstack((vx_buf, np.mean(_t2n(vx))))
        vy_buf = np.hstack((vy_buf, np.mean(_t2n(vy))))
        vz_buf = np.hstack((vz_buf, np.mean(_t2n(vz))))
        ...
        lef_buf = np.hstack((lef_buf, np.mean(_t2n(lef))))
    ...
    np.save('./result/ax.npy', vx_buf)
    np.save('./result/ay.npy', vy_buf)
    np.save('./result/az.npy', vz_buf)
    ...
    np.save('./result/lef.npy', lef_buf)
