"""
PPO+PID vs PID comparison for Simulink angle tracking — NEW task (θ₀=0, target∈[-2,+2]).
PPO controls until relative error < 2%, then PID takes over for the rest of the episode.

Three figures displayed one by one:
  Fig 1: SSE vs target value (100 episodes, 5000 steps each)
  Fig 2: θ tracking curves  (first 10 episodes, 5000 steps each)
  Fig 3: Control output      (first 10 episodes, 5000 steps each)
"""
import os
import sys
...
SEED = 42
N_TOTAL = 10
        # total episodes for SSE comparison
...
DEAD_ZONE = 0.03     # reject targets in (-0.03, 0.03)
...
PID_SWITCH_THRESHOLD = 0.02  # 2% relative error → switch PPO to PID

# PPO model path — update to latest new-task checkpoint
PPO_RUN_DIR = "/home/a/demo/NeuralPlane_stable_V2/scripts/runs/2026-04-17_12-13-24_Control_simulink_Simulink_ppo_v1"
PPO_EPISODE_DIR = os.path.join(PPO_RUN_DIR, "episode_82")
...
# ===================== Run PPO+PID — 100 episodes × 5000 steps =====================
# Pre-generate targets in [-2,+2] with dead zone rejection
def _gen_target(rng, deadzone=DEAD_ZONE):
    while True:
        t = rng.random() * 4.0 - 2.0  # uniform [-2, +2]
        if abs(t) >= deadzone:
            return t

target_rng = np.random.RandomState(SEED)
episode_targets = [_gen_target(target_rng) for _ in range(N_TOTAL)]
...
for ep in range(N_TOTAL):
    ...
    # Override target (env already reset to θ₀=0)
    ...
    masks = torch.zeros(1, 1, device=torch.device(device))  # new episode

    ep_theta = [float(_t2n(env_ppo.model.s[:, 3]).item())]
    ep_appc = [float(_t2n(env_ppo.model.u[:, 0]).item())]
    bad = False
    use_pid = False  # PID takeover flag for this episode
    pid_helper = PIDController(Kp=Kp, Ki=Ki, Kd=Kd, dt=dt)
    step_mag = float(_t2n(env_ppo.task.step_magnitude).item())

    # Disable env-level PID takeover — we handle it ourselves
    env_ppo.task.pid_active[:] = False
    ...
    for step in range(STEPS_PER_EP):
        if not use_pid:
            ...
            # Check if PPO brought error within 2% → switch to PID
            theta_now = float(_t2n(env_ppo.model.s[:, 3]).item())
            rel_err = abs(theta_now - target) / max(step_mag, 0.03)
            if rel_err < PID_SWITCH_THRESHOLD:
                use_pid = True
                pid_helper.reset()

        if use_pid:
            theta_now = float(_t2n(env_ppo.model.s[:, 3]).item())
            q_now = float(_t2n(env_ppo.model.s[:, 2]).item())
            error = target - theta_now
            action_val = pid_helper.compute(error, q_now)
            actions = torch.tensor([[action_val / 0.35]], dtype=torch.float32, device=device)

        # Disable env-level PID before stepping
        env_ppo.task.pid_active[:] = False
        obs, rew, done, bad_done, exceed, info = env_ppo.step(actions)
        env_ppo.task.pid_active[:] = False  # prevent env from activating it
        ...
        # Prevent env auto-reset on next step
...
# ===================== Run PID — 100 episodes × 5000 steps =====================
print("\n" + "=" * 60)
...
for ep in range(N_TOTAL):
    ...
    for step in range(STEPS_PER_EP):
        ...
        # Disable env-level PID takeover — this loop is already pure PID
        env_pid.task.pid_active[:] = False
...
for i in range(N_TOTAL):
    if abs(ppo_data[i]['target'] - pid_data[i]['target']) > 1e-6:
        mismatch += 1
        if mismatch <= 3:
            print(f"WARNING: target mismatch Ep {i+1}: "
                  f"PPO={ppo_data[i]['target']:.6f} PID={pid_data[i]['target']:.6f}")
if mismatch:
    print(f"WARNING: {mismatch}/{N_TOTAL} targets mismatched!")
else:
    print(f"\n✓ All {N_TOTAL} targets match between PPO and PID.")
...
print(f"\nPPO+PID: mean SSE = {ppo_mean_sse:.6f}, diverged = {ppo_bad}/{N_TOTAL}")
print(f"PID: mean SSE = {pid_mean_sse:.6f}, diverged = {pid_bad}/{N_TOTAL}")

# ===================== Figure 1: SSE vs Target Value =====================
...
try:
    ...
    ax1.plot(x_smooth, spl_ppo(x_smooth), color='tab:red', linewidth=1.5, label='PPO+PID')
    ax1.plot(x_smooth, spl_pid(x_smooth), color='tab:blue', linewidth=1.5, label='PID')
except Exception:
    ax1.plot(x_sorted, ppo_sse[sort_idx], color='tab:red', linewidth=1.0, label='PPO+PID')
...
ax1.set_title(f'PPO+PID vs PID — Steady-State Error (θ₀=0, target∈[-2,+2], {STEPS_PER_EP} steps)',
              fontsize=14, fontweight='bold')
ax1.legend(fontsize=11)
ax1.grid(True, alpha=0.3)
plt.tight_layout()
fig1.savefig(os.path.join(result_dir, 'compare_sse_vs_target.png'), dpi=150)
print(f"  已保存: {result_dir}/compare_sse_vs_target.png")
plt.show()

# ===================== Figure 2: θ Tracking Curves (10 episodes) =====================
print("\n绘制 Figure 2: θ 跟踪曲线 (前10个episode) ...")

fig2, axes2 = plt.subplots(2, 5, figsize=(22, 8), sharex=True)
fig2.suptitle(f'PPO+PID vs PID — θ Tracking ({N_PLOT} episodes, {STEPS_PER_EP} steps)',
...
for i in range(N_PLOT):
    ...
    ax.plot(t_ppo, ppo_data[i]['theta'], color='tab:red', linewidth=1.0, label='PPO+PID')
...
plt.tight_layout()
fig2.savefig(os.path.join(result_dir, 'compare_tracking.png'), dpi=150)
print(f"  已保存: {result_dir}/compare_tracking.png")
plt.show()

# ===================== Figure 3: Control Output Curves (10 episodes) =====================
print("\n绘制 Figure 3: 控制输出曲线 (前10个episode) ...")

fig3, axes3 = plt.subplots(2, 5, figsize=(22, 8), sharex=True)
fig3.suptitle(f'PPO+PID vs PID — Control Output ({N_PLOT} episodes, {STEPS_PER_EP} steps)',
              fontsize=15, fontweight='bold')
...
for i in range(N_PLOT):
    ...
    # Skip t=0 point to avoid initial jump artifact
    ax.plot(t_ppo[1:], ppo_data[i]['appc'][1:], color='tab:red', linewidth=0.8, label='PPO+PID', alpha=0.85)
...
plt.tight_layout()
fig3.savefig(os.path.join(result_dir, 'compare_control.png'), dpi=150)
print(f"  已保存: {result_dir}/compare_control.png")
plt.show()
