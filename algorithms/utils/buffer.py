...
class ReplayBuffer(Buffer):
    ...
    def advantages(self) -> np.ndarray:
        ...
        # contain_nan = (True in np.isnan(advantages - advantages.mean()))
        # if contain_nan:
        #     pdb.set_trace()
        ...
        if self.use_proper_time_limits:
            ...
            if self.use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    td_delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                    # 设置终局奖励
                    # td_delta += -100 * (1 - self.bad_masks[step + 1]) + 200 * (1 - self.masks[step + 1])
                    gae = td_delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                    self.returns[step] = gae + self.value_preds[step]
            else:
                self.returns[-1] = next_value
    ...
    def feed_forward_generator(buffer: Union[Buffer, List[Buffer]], num_mini_batch: int):
        """
        A feed-forward generator that yields training data arranged in randomly shuffled mini batches.

        Args:
            buffer (Buffer or List[Buffer])
        ...
        assert all([b.n_rollout_threads == n_rollout_threads for b in buffer]) \
            ...
            "Input buffers must have the same type and shape"
...
class AdvReplayBuffer(Buffer):
    ...
    def __init__(self, args, num_agents, obs_space, act_space):
        ...
        self.obs = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, *self.obs_shape), dtype=np.float32)
        self.actions = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, *self.act_shape), dtype=np.float32)
        self.reset = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, 1), dtype=bool)
        self.rewards = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
    ...
    def insert(self, 
        ...
        self.actions[self.step] = actions.copy()
        self.reset[self.step] = reset.copy()
        self.rewards[self.step] = rewards.copy()
        self.step = (self.step + 1) % self.buffer_size
    ...
    def clear(self):
        ...
        self.actions = np.zeros_like(self.actions, dtype=np.float32)
        self.rewards = np.zeros_like(self.rewards, dtype=np.float32)
        self.reset = np.zeros_like(self.reset, dtype=bool)
        self.returns = np.zeros_like(self.returns, dtype=np.float32)

    def compute_returns(self):
        self.returns = np.zeros_like(self.returns, dtype=np.float32)
        for step in reversed(range(self.rewards.shape[0])):
            mask_back = np.ones_like(self.reset[step], dtype=bool)
            for i in range(self.horizon):
                if step-i >= 0:
                    self.returns[step-i] += self.rewards[step] * mask_back
                    if step-i-1 >= 0:
                        mask_back = mask_back * self.reset[step-i-1]

    def mini_batch_generator(self, num_mini_batches, num_epochs = 8):
        batch_size = self.buffer_size * self.n_rollout_threads - self.horizon
        mini_batch_size = batch_size // num_mini_batches
        indices = np.random.permutation(num_mini_batches * mini_batch_size)

        obs = self.obs.reshape(self.buffer_size * self.n_rollout_threads, self.num_agents, *self.obs_shape).squeeze(1)
        actions = self.actions.reshape(self.buffer_size * self.n_rollout_threads, self.num_agents, *self.act_shape).squeeze(1)
        returns = self.returns.reshape(self.buffer_size * self.n_rollout_threads, self.num_agents, 1).squeeze(1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                target_values_batch = returns[batch_idx]
                yield obs_batch, actions_batch, target_values_batch
