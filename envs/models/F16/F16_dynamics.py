...
class F16Dynamics(nn.Module):
    ...
    def nlplant(self, x):
        ...
        Udot = R * V - Q * W - g * st + qbar * S * Cx_tot / self.m + T / self.m #机体坐标系,这里的推力做了很强的假设，只作用于一个方向
        Vdot = P * W - R * U + g * ct * sphi + qbar * S * Cy_tot / self.m
        Wdot = Q * U - P * V + g * ct * cphi + qbar * S * Cz_tot / self.m
