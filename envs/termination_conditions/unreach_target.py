...
class UnreachTarget(BaseTerminationCondition):
    ...
    def get_termination(self, task, env, info={}):
        ...
        mask_alt = (altitude > torch.maximum(task.target_altitude+100,task.target_altitude-task.delta_altitude+100)) | (altitude < torch.minimum(task.target_altitude-100,task.target_altitude-task.delta_altitude-100))

        # 判断时间
        mask1 = (check_time >= self.max_check_interval) | (distance > task.max_distance*1.5) | (npos > task.target_npos + 100) | mask_alt | (torch.abs(epos) > torch.abs(task.target_epos) + 100)
