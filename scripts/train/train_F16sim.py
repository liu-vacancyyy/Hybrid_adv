...
def make_train_env(all_args):
    def get_env_fn():
        def init_env():
            if all_args.env_name == "Control": #代表端到端控制，目前任务都使用的这个环境
...
def main(args):
    ...
    # env init
    envs = make_train_env(all_args) #构建训练环境
    eval_envs = make_eval_env(all_args) if all_args.use_eval else None #构建评估环境，一般是不在训练时开启
    ...
    if all_args.use_selfplay:
        ...
        runner = F16SimRunner(config, writer) #一般只使用这一个runner
    try:
        runner.run()
