#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# PPO-GRU training for rc_human from scratch with PX4-style RC command
# generation and an explicit hard raw-stick rate limit.
#
# This script intentionally starts a fresh policy:
#   - no --model-dir
#   - no --init-actor-ckpt
#
# Defaults are chosen for the rate-limited command task:
#   - RC_HUMAN_COMMAND_RATE_LIMIT_FRAC=0.05 limits each raw stick channel to
#     0.05 per 20 ms step: 0->1 in 0.4 s, -1->1 in 0.8 s.
#   - NUM_ENV_STEPS=2.5e9 gives the adaptive curriculum more room to reach
#     mode3/mode4 after the easier modes stabilize.
#   - LR=2e-4, VALUE_LOSS_COEF=0.5, and clipped value loss reduce critic spikes.
#
# Run:
#   cd /home/a/demo/Hybrid_adv
#   bash scripts/train_rc_human_rl_rate_limited_from_scratch.sh
#
# Useful overrides:
#   DEVICE=cuda:0 SEED=31 RC_HUMAN_COMMAND_RATE_LIMIT_FRAC=0.03 \
#   NUM_ENV_STEPS=3.0e9 RC_HUMAN_EXP_NAME=rc_human_rl_rate003 \
#   bash scripts/train_rc_human_rl_rate_limited_from_scratch.sh
# ---------------------------------------------------------------------------
set -euo pipefail
...
EXP="${RC_HUMAN_EXP_NAME:-rc_human_rl_px4_rate${RATE_TAG}_from_scratch_modes012534}"
...
echo "rc_human rate-limited from-scratch training"
echo "  experiment: ${EXP}"
