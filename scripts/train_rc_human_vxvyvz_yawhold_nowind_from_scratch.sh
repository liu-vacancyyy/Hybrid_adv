#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# PPO-GRU training for rc_human vx/vy/vz tracking with yaw-hold, no wind.
#
# This preset keeps yaw command disabled.  target_vx/target_vy are interpreted
# in the current-heading local frame, while target_heading is used only for
# yaw-hold tracking.
#
# Run directly for all modes:
#   cd /home/a/demo/Hybrid_adv
#   bash scripts/train_rc_human_vxvyvz_yawhold_nowind_from_scratch.sh
#
# Or use the mode-range wrappers:
#   bash scripts/train_rc_human_vxvyvz_yawhold_modes0_1_nowind_from_scratch.sh
#   bash scripts/train_rc_human_vxvyvz_yawhold_modes0_3_nowind_from_scratch.sh
#   bash scripts/train_rc_human_vxvyvz_yawhold_modes0_5_nowind_from_scratch.sh
# ---------------------------------------------------------------------------
set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)

export RC_HUMAN_MODE_ORDER="${RC_HUMAN_MODE_ORDER:-0 1 2 5 4 3}"
export RC_HUMAN_MAX_MODE_SLOTS="${RC_HUMAN_MAX_MODE_SLOTS:-6}"
export RC_HUMAN_CURRICULUM_ENABLE="${RC_HUMAN_CURRICULUM_ENABLE:-1}"
export RC_HUMAN_VX_FORWARD_CURRICULUM_ENABLE="${RC_HUMAN_VX_FORWARD_CURRICULUM_ENABLE:-0}"
export RC_HUMAN_COMMAND_RATE_LIMIT_FRAC="${RC_HUMAN_COMMAND_RATE_LIMIT_FRAC:-0.05}"
export RC_HUMAN_MIX_CURRENT="${RC_HUMAN_MIX_CURRENT:-0.25}"
export RC_HUMAN_MIX_EASY_REPLAY="${RC_HUMAN_MIX_EASY_REPLAY:-0.25}"
export RC_HUMAN_MIX_MEDIUM_REPLAY="${RC_HUMAN_MIX_MEDIUM_REPLAY:-0.25}"
export RC_HUMAN_MIX_RANDOM_REPLAY="${RC_HUMAN_MIX_RANDOM_REPLAY:-0.25}"

export RC_HUMAN_VX_MIN="${RC_HUMAN_VX_MIN:--1.0}"
export RC_HUMAN_VX_MAX="${RC_HUMAN_VX_MAX:-1.0}"
export RC_HUMAN_VX_LIMIT="${RC_HUMAN_VX_LIMIT:-1.0}"
export RC_HUMAN_VY_LIMIT="${RC_HUMAN_VY_LIMIT:-1.0}"
export RC_HUMAN_VZ_LIMIT="${RC_HUMAN_VZ_LIMIT:-1.0}"

export RC_HUMAN_YAW_COMMAND_ENABLE="${RC_HUMAN_YAW_COMMAND_ENABLE:-0}"
export RC_HUMAN_YAW_HOLD_ENABLE="${RC_HUMAN_YAW_HOLD_ENABLE:-1}"
export RC_HUMAN_YAW_TRACKING_ENABLE="${RC_HUMAN_YAW_TRACKING_ENABLE:-1}"
export RC_HUMAN_YAW_REWARD_ENABLE="${RC_HUMAN_YAW_REWARD_ENABLE:-1}"
export RC_HUMAN_YAW_REWARD_MODE="${RC_HUMAN_YAW_REWARD_MODE:-target}"

export RC_HUMAN_TRACKING_BAD_DONE_ENABLE="${RC_HUMAN_TRACKING_BAD_DONE_ENABLE:-0}"
export RC_HUMAN_VXYVZ_DYNAMIC_BAD_DONE_ENABLE="${RC_HUMAN_VXYVZ_DYNAMIC_BAD_DONE_ENABLE:-0}"
export RC_HUMAN_VXYVZ_DYNAMIC_BAD_DONE_MARGIN="${RC_HUMAN_VXYVZ_DYNAMIC_BAD_DONE_MARGIN:-0.5}"
export RC_HUMAN_ALTITUDE_AWARE_VZ_ENABLE="${RC_HUMAN_ALTITUDE_AWARE_VZ_ENABLE:-1}"
export RC_HUMAN_ALT_LOW="${RC_HUMAN_ALT_LOW:-0.5}"
export RC_HUMAN_ALT_HIGH="${RC_HUMAN_ALT_HIGH:-10.0}"
export RC_HUMAN_ALT_GUARD_ZONE="${RC_HUMAN_ALT_GUARD_ZONE:-0.0}"

export RC_HUMAN_W_VEL="${RC_HUMAN_W_VEL:-8.0}"
export RC_HUMAN_W_YAW="${RC_HUMAN_W_YAW:-3.0}"
export RC_HUMAN_W_YAW_PRECISION="${RC_HUMAN_W_YAW_PRECISION:-0.4}"
export RC_HUMAN_W_YAW_RATE="${RC_HUMAN_W_YAW_RATE:-0.6}"
export RC_HUMAN_W_ATTITUDE="${RC_HUMAN_W_ATTITUDE:-1.6}"
export RC_HUMAN_W_OMEGA="${RC_HUMAN_W_OMEGA:-1.2}"
export RC_HUMAN_W_SMOOTH="${RC_HUMAN_W_SMOOTH:-0.5}"
export RC_HUMAN_W_VEL_PRECISION="${RC_HUMAN_W_VEL_PRECISION:-0.0}"
export RC_HUMAN_REL_ERROR_FIXED_SCALE="${RC_HUMAN_REL_ERROR_FIXED_SCALE:-1.0}"
export RC_HUMAN_REL_ERROR_FLOOR="${RC_HUMAN_REL_ERROR_FLOOR:-0.20}"
export RC_HUMAN_W_REL_TRACKING="${RC_HUMAN_W_REL_TRACKING:-1.8}"
export RC_HUMAN_W_REL_PRECISION="${RC_HUMAN_W_REL_PRECISION:-0.8}"
export RC_HUMAN_REL_PRECISION_GAIN="${RC_HUMAN_REL_PRECISION_GAIN:-12.0}"
export RC_HUMAN_W_OVERSHOOT="${RC_HUMAN_W_OVERSHOOT:-0.5}"
export RC_HUMAN_OVERSHOOT_DEADBAND="${RC_HUMAN_OVERSHOOT_DEADBAND:-0.08}"
export RC_HUMAN_W_ADAPTIVE_DAMPING="${RC_HUMAN_W_ADAPTIVE_DAMPING:-0.08}"
export RC_HUMAN_ADAPTIVE_DAMPING_GAIN="${RC_HUMAN_ADAPTIVE_DAMPING_GAIN:-10.0}"
export RC_HUMAN_W_SPEED_MARGIN="${RC_HUMAN_W_SPEED_MARGIN:-2.0}"
export RC_HUMAN_SPEED_MARGIN_START="${RC_HUMAN_SPEED_MARGIN_START:-1.7320508075688772}"
export RC_HUMAN_SPEED_MARGIN_SCALE="${RC_HUMAN_SPEED_MARGIN_SCALE:-1.0}"
export RC_HUMAN_W_ATTITUDE_MARGIN="${RC_HUMAN_W_ATTITUDE_MARGIN:-2.0}"
export RC_HUMAN_ROLL_MARGIN_START_DEG="${RC_HUMAN_ROLL_MARGIN_START_DEG:-20.0}"
export RC_HUMAN_PITCH_MARGIN_START_DEG="${RC_HUMAN_PITCH_MARGIN_START_DEG:-15.0}"
export RC_HUMAN_ATTITUDE_MARGIN_SCALE_DEG="${RC_HUMAN_ATTITUDE_MARGIN_SCALE_DEG:-5.0}"

export RC_HUMAN_SIG_VX="${RC_HUMAN_SIG_VX:-0.25}"
export RC_HUMAN_SIG_VY="${RC_HUMAN_SIG_VY:-0.25}"
export RC_HUMAN_SIG_VZ="${RC_HUMAN_SIG_VZ:-0.22}"
export RC_HUMAN_SIG_YAW="${RC_HUMAN_SIG_YAW:-0.12}"
export RC_HUMAN_SIG_YAW_DELTA="${RC_HUMAN_SIG_YAW_DELTA:-0.015}"
export RC_HUMAN_SIG_YAW_RATE="${RC_HUMAN_SIG_YAW_RATE:-0.35}"
export RC_HUMAN_SIG_ATTITUDE="${RC_HUMAN_SIG_ATTITUDE:-0.10}"
export RC_HUMAN_SIG_OMEGA="${RC_HUMAN_SIG_OMEGA:-0.70}"
export RC_HUMAN_SIG_SMOOTH="${RC_HUMAN_SIG_SMOOTH:-0.35}"

export RC_HUMAN_SUCCESS_VEL_ERROR="${RC_HUMAN_SUCCESS_VEL_ERROR:-0.18}"
export RC_HUMAN_SUCCESS_YAW_ERROR="${RC_HUMAN_SUCCESS_YAW_ERROR:-0.12}"
export RC_HUMAN_SUCCESS_ATTITUDE_ERROR="${RC_HUMAN_SUCCESS_ATTITUDE_ERROR:-0.12}"
export RC_HUMAN_SUCCESS_USE_YAW_ATTITUDE="${RC_HUMAN_SUCCESS_USE_YAW_ATTITUDE:-0}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE0="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE0:-0.10}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE1="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE1:-0.25}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE2="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE2:-0.35}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE3="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE3:-0.45}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE4="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE4:-0.35}"
export RC_HUMAN_SUCCESS_VEL_ERROR_MODE5="${RC_HUMAN_SUCCESS_VEL_ERROR_MODE5:-0.35}"

export RC_HUMAN_ENABLE_SENSOR_NOISE="${RC_HUMAN_ENABLE_SENSOR_NOISE:-1}"
export RC_HUMAN_SENSOR_POS_STD="${RC_HUMAN_SENSOR_POS_STD:-0.02}"
export RC_HUMAN_SENSOR_VEL_STD="${RC_HUMAN_SENSOR_VEL_STD:-0.02}"
export RC_HUMAN_SENSOR_ATT_STD="${RC_HUMAN_SENSOR_ATT_STD:-0.005}"
export RC_HUMAN_SENSOR_OMEGA_STD="${RC_HUMAN_SENSOR_OMEGA_STD:-0.0005}"

export RC_HUMAN_INIT_ROLL_RANGE="${RC_HUMAN_INIT_ROLL_RANGE:-0.0}"
export RC_HUMAN_INIT_PITCH_RANGE="${RC_HUMAN_INIT_PITCH_RANGE:-0.0}"
export RC_HUMAN_INIT_YAW_RANGE="${RC_HUMAN_INIT_YAW_RANGE:-3.141592653590}"
export RC_HUMAN_INIT_VEL_RANGE="${RC_HUMAN_INIT_VEL_RANGE:-0.0}"
export RC_HUMAN_INIT_OMEGA_RANGE="${RC_HUMAN_INIT_OMEGA_RANGE:-0.02}"
export RC_HUMAN_DR_MASS="${RC_HUMAN_DR_MASS:-0.05}"
export RC_HUMAN_DR_INERTIA="${RC_HUMAN_DR_INERTIA:-0.20}"
export RC_HUMAN_MIN_ALTITUDE="${RC_HUMAN_MIN_ALTITUDE:-1.5}"
export RC_HUMAN_MAX_ALTITUDE="${RC_HUMAN_MAX_ALTITUDE:-8.5}"
export RC_HUMAN_MAX_VELOCITY="${RC_HUMAN_MAX_VELOCITY:-5.0}"

export NUM_ENV_STEPS="${NUM_ENV_STEPS:-2.0e9}"
export SAVE_INTERVAL="${SAVE_INTERVAL:-5}"
export ENTROPY_COEF="${ENTROPY_COEF:-1e-3}"

RATE_TAG=$(printf '%s' "${RC_HUMAN_COMMAND_RATE_LIMIT_FRAC}" | tr -c '0-9A-Za-z' '_')
MODE_TAG=$(printf '%s' "${RC_HUMAN_MODE_ORDER}" | tr -c '0-9A-Za-z' '_')
export NO_WIND_CONFIG_NAME="${NO_WIND_CONFIG_NAME:-rc_human_vxvyvz_yawhold_currheading_modes${MODE_TAG}_nowind_runtime}"
export RC_HUMAN_EXP_NAME="${RC_HUMAN_EXP_NAME:-rc_human_vxvyvz_yawhold_currheading_rate${RATE_TAG}_from_scratch_modes${MODE_TAG}}"

exec bash "${SCRIPT_DIR}/train_rc_human_rl_nowind_from_scratch.sh" "$@"
