...
python train/train_F16sim_adv.py \
    --env-name ${env} --algorithm-name ${algo} --scenario-name ${scenario} --model-name ${model} --experiment-name ${exp} \
    --seed ${seed} --device ${device} --n-training-threads 1 --n-rollout-threads 832 --cuda \
    --log-interval 1 --save-interval 10 \
    --num-mini-batch 5 --buffer-size 3000 --num-env-steps 2e9 \
